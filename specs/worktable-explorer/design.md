...
**Provenance:** ported verbatim (byte-for-byte `views/worktable-explorer/index.js`) from the `segreteria-campo` plugin's `worktable-explorer` SPA. The entry point (`app-worktable-explorer.js`) and PHP mount (`dashboard-worktable-explorer.inc.php`) were rewritten to match this plugin's conventions (manual mount, per-SPA `?v=<filemtime>` cache-busting on the boot script — see AGENTS.md).
...
Verified against this app's `camila/js/worktable-client.js`: `tables()`, `uploadAttachment`, `fetchAttachment`, `listAttachments`, `deleteAttachment` are all present with the same signatures as in the source plugin.
