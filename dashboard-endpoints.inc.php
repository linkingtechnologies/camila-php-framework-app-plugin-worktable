<?php
// Endpoints dashboard — worktable plugin
// Shows this app's own MCP endpoint URL. Formerly the plugin's default "m0"
// tab; the default landing tab is now the separate "home" dashboard.
...
$pluginI18n = [
    'endpoints.mcpEndpoint.label'   => $lang['endpoints.mcpEndpoint.label'] ?? '',
    'endpoints.restApi.label'       => $lang['endpoints.restApi.label'] ?? '',
    'endpoints.btn.copy'            => $lang['endpoints.btn.copy'] ?? '',
    'endpoints.copied'              => $lang['endpoints.copied'] ?? '',
    'endpoints.copyError'           => $lang['endpoints.copyError'] ?? '',
    'endpoints.claudeDesktop.title'  => $lang['endpoints.claudeDesktop.title'] ?? '',
    'endpoints.claudeDesktop.step1'  => $lang['endpoints.claudeDesktop.step1'] ?? '',
    'endpoints.claudeDesktop.step2'  => $lang['endpoints.claudeDesktop.step2'] ?? '',
    'endpoints.claudeDesktop.configLabel' => $lang['endpoints.claudeDesktop.configLabel'] ?? '',
    'endpoints.openai.title'        => $lang['endpoints.openai.title'] ?? '',
    'endpoints.openai.step1'        => $lang['endpoints.openai.step1'] ?? '',
    'endpoints.openai.step2'        => $lang['endpoints.openai.step2'] ?? '',
    'endpoints.openai.step3'        => $lang['endpoints.openai.step3'] ?? '',
    'endpoints.openai.limits'       => $lang['endpoints.openai.limits'] ?? '',
    'endpoints.openai.warnNotPublic' => $lang['endpoints.openai.warnNotPublic'] ?? '',
    'endpoints.openai.warnAuth'     => $lang['endpoints.openai.warnAuth'] ?? '',
];
...
$endpointsScriptVersion = @filemtime(__DIR__ . '/app-endpoints.js');
$endpointsVerSuffix     = $endpointsScriptVersion ? ('?v=' . $endpointsScriptVersion) : '';
$_CAMILA['page']->camila_add_js('<script type="module" src="./plugins/worktable/app-endpoints.js' . $endpointsVerSuffix . '"></script>');
